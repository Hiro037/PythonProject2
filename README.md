...
for card in gen:
    print(card)
```
## Функции импорта транзакций

Проект включает две функции для считывания финансовых транзакций из файлов:

- **read_csv_transactions(file_path: str) -> list[dict]**  
  Считывает финансовые транзакции из CSV-файла по указанному пути.  
  **Аргументы:**  
  - `file_path` — путь к CSV-файлу.  
  **Возвращает:**  
  - Список словарей, где каждый словарь представляет отдельную транзакцию.  
  **Особенности:**  
  - Если файл не найден или происходит ошибка при чтении, функция выводит сообщение об ошибке и возвращает пустой список.

- **read_excel_transactions(file_path: str) -> list[dict]**  
  Считывает финансовые транзакции из Excel-файла по указанному пути, используя библиотеку `pandas`.  
  **Аргументы:**  
